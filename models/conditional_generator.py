...
class ConditionalGenerator(nn.Module):
    ...
    def _init_diff(self, configs):
        ...
        if "aireadi" in self.cond_configs["cond_modal"]:
            configs["diffusion"]["aireadi_projector"] = self.cond_configs["aireadi"]["aireadi_projector"]
    ...
    def forward(self, batch, is_train):
        ...
        x, tp, attrs_emb_raw = self._unpack_data_cond_gen(batch)
        ...
        if is_train:
            ...
            attr_emb = self.cond_projector(attrs_emb_raw)  # for now we are not using projector.
        ...
        # if "multimodal" in self.cond_configs["cond_modal"]:
        #     attr_emb = self.cond_projector(attr_emb_raw)
        attr_emb = self.cond_projector(attrs_emb_raw)
    ...
    def _unpack_data_cond_gen(self, batch):
        if "aireadi" in self.cond_configs["cond_modal"]:
            ...
            B, _, T = ts.shape
            tp = torch.arange(T).repeat(B, 1).to(self.device).float()
            attrs_embed = batch['text_embedding_all_segments'].mean(1)
    ...
    def generate_text(self, batch, n_samples, sampler="ddim"):
        ...
        ts, tp, attr_emb_raw = self._unpack_data_cond_gen(batch)
