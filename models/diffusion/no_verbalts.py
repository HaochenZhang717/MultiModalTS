...
class TsPatchEmbedding(nn.Module):
    ...
    def forward(self, x_in):
        ...
        if x_in.shape[-1] % self.L_patch_len:
            ...
            x = x_in
        x = x.unfold(dimension=3, size=self.L_patch_len,
                     step=self.L_patch_len)  # (B, c, n_var, t/L_patch_len, L_patch_len)
...
class NoVerbalTS(nn.Module):
    ...
    def forward(self, x_raw, tp, diffusion_step):
        ...
        for layer in self.residual_layers:
            x_in, skip_connection = layer(x_in + _x_in, side_in, diffusion_emb)
